...
listDevice=[
{'SKU' : 0, 'BRAND' : 'ASUS', 'TYPE' : 'HANDHELD', 'MODEL': 'ROG ALLY X', 'PANEL(Hz)' : 120, 'CPU' : 'Z1 Extreme', 'GPU' : 'AMD Radeon 780M', 'RAM' : '24 GB', 'SSD' : '1 TB', 'PRICE $' : 750},
...
{'SKU' : 3, 'BRAND' : 'LENOVO', 'TYPE' : 'LAPTOP', 'MODEL':'ThinkPad X1 Carbon', 'PANEL(Hz)' : 60, 'CPU' : 'Intel Core i7-1260P', 'GPU' : 'Intel Iris Xe', 'RAM' : '16 GB', 'SSD' : '1 TB', 'PRICE $' : 1300},
...
{'SKU' : 5, 'BRAND' : 'HP', 'TYPE' : 'LAPTOP', 'MODEL':'Omen 6', 'PANEL(Hz)' : 165, 'CPU' : 'AMD Ryzen 9 7940HS', 'GPU' : 'RTX 4060 M', 'RAM' : '32 GB', 'SSD' : '1 TB', 'PRICE $' : 1950},
    ...
    while True:
        try:
            ...
            if inputSaldo < 0:
                ...
                saldo += inputSaldo
                print(f"\nSaldo uang Anda sekarang: ${saldo:}")
                break  
        except ValueError:
...
# sort function untuk termurah dan termahal - extra feature ========================================

def sort_by_harga(device_list, order='min'):
    
    if order == 'min':
        sorted_list = sorted(device_list, key=lambda x: x['PRICE $'])
    elif order == 'max':
        sorted_list = sorted(device_list, key=lambda x: x['PRICE $'], reverse=True)
    
    table = tabulate(sorted_list, headers="keys", tablefmt="fancy_grid")
    print(table)

def harga_termurah():
    print("\nDevices di sortir berdasarkan harga termurah:")
    sort_by_harga(listDevice, order='min')

def harga_termahal():
    print("\nDevices di sortir berdasarkan harga termahal:")
    sort_by_harga(listDevice, order='max')
...
def showList(): 
    while True:
        opsi= input(''' 
              ...
              5. belum yakin ? yuk lihat rekomendasi berdasarkan harga termurah & termahal
...
def select_and_update_device():
    ...
    if device:
        ...
        print()
        if konfirmasi != 'yes':
            print(f"\t tidak jadi melakukan edit untuk no sku {sku} .")
            return
